...
namespace Business.Managers.ProductEvent.Update
{
    public partial class UpdateProductCommandHandler : IRequestHandler<UpdateProductCommandQuery, ResponseDataResult<Product>>
    {
        ...
        public async Task<ResponseDataResult<Product>> Handle(UpdateProductCommandQuery request, CancellationToken cancellation)
        {
            var GetModel = await _service.GetAllAsync( x => x.ID == request.ProductID);
